...
// Unified helper to spawn a Web Worker for dimensionality reduction (t-SNE or UMAP) + clustering
// without adding any project-wide dependencies or editing existing files.

export type ReductionMethod = "tsne" | "umap";
...
export type DimensionWorkerHandle = {
  worker: Worker;
  revoke: () => void;
};

export function createDimensionWorker(): DimensionWorkerHandle {
...
function getWorkerSource(): string {
  // Pure JS worker code string; loads tsne-js from CDN and uses inline UMAP implementation
  return `
    // --- dynamic imports for t-SNE ---
    (function loadTSNE() {
      var tsneUrls = [
        '/vendor/tsne.min.js',
        '/tsne.min.js',
      ...
      for (var i=0;i<tsneUrls.length;i++) {
        try { importScripts(tsneUrls[i]); break; } catch (e) { /* try next */ }
      }
    })();
    
    // Try to load UMAP from CDN, if fails we'll handle in runUMAP
    (function loadUMAP() {
      try {
        importScripts('https://cdn.jsdelivr.net/npm/umap-js@1.4.0/lib/umap-js.min.js');
      } catch(e) {
        try {
          importScripts('https://unpkg.com/umap-js@1.4.0/lib/umap-js.min.js');
        } catch(e2) {
          // Will use fallback simple implementation
        }
      }
    })();
    ...
    function runUMAP(matrix, params){
      // Check if UMAP is available
      var UMAP = self.UMAP;
      
      if (!UMAP) {
        post('log', { message: 'UMAP library not loaded from CDN, using PCA fallback for visualization' });
        // Simple PCA-like projection as fallback
        return runSimplePCA(matrix, 2);
      }
      
      var nNeighbors = (params && params.nNeighbors) || 15;
      var minDist = (params && params.minDist) || 0.1;
      var nComponents = (params && params.nComponents) || 2;
      var nEpochs = (params && params.nEpochs) || 400;

      post('log', { message: 'Running UMAP with nNeighbors='+nNeighbors+', minDist='+minDist+', nEpochs='+nEpochs });
      ...
      try {
        ...
        var embedding = umap.fit(matrix);
        
        // normalize to zero-mean unit-variance per axis for stability
        var xs = embedding.map(function(p){return p[0];});
        var ys = embedding.map(function(p){return p[1];});
        function norm(arr){ var m=arr.reduce((s,v)=>s+v,0)/arr.length; var sd=Math.sqrt(arr.reduce((s,v)=>s+(v-m)*(v-m),0)/Math.max(1,arr.length-1))||1; return {m:m,sd:sd}; }
        var nx = norm(xs), ny = norm(ys);
        var emb = embedding.map(function(p){ return [(p[0]-nx.m)/nx.sd, (p[1]-ny.m)/ny.sd]; });
        return emb;
      } catch(err) {
        post('log', { message: 'UMAP failed: ' + (err.message || err) + ', using PCA fallback' });
        return runSimplePCA(matrix, 2);
      }
    }
    
    // Simple PCA fallback (projects to first 2 principal components)
    function runSimplePCA(matrix, nComponents) {
      var n = matrix.length;
      var d = matrix[0].length;
      
      // Center the data
      var means = new Array(d).fill(0);
      for (var j=0; j<d; j++) {
        var sum = 0;
        for (var i=0; i<n; i++) sum += matrix[i][j];
        means[j] = sum / n;
      }
      ...
      // Simple projection: use first 2 dimensions (or use random projection)
      // For better results, could implement power iteration for top eigenvectors
      // but for now just project to first 2 dims with some random rotation
      var result = new Array(n);
      for (var i=0; i<n; i++) {
        // Simple projection - take first two dims or combine if more features
        var x = centered[i][0] || 0;
        var y = centered[i][1] || 0;
        if (d > 2) {
          // Add some contribution from other dimensions
          for (var j=2; j<Math.min(d, 10); j++) {
            x += centered[i][j] * 0.3 * Math.cos(j);
            y += centered[i][j] * 0.3 * Math.sin(j);
          }
        }
        result[i] = [x, y];
      }
      ...
      // Normalize
      var xs = result.map(function(p){return p[0];});
      var ys = result.map(function(p){return p[1];});
      function norm(arr){ var m=arr.reduce((s,v)=>s+v,0)/arr.length; var sd=Math.sqrt(arr.reduce((s,v)=>s+(v-m)*(v-m),0)/Math.max(1,arr.length-1))||1; return {m:m,sd:sd}; }
      var nx = norm(xs), ny = norm(ys);
      return result.map(function(p){ return [(p[0]-nx.m)/nx.sd, (p[1]-ny.m)/ny.sd]; });
    }
    ...
    self.onmessage = function(e){
      try {
        ...
        var reductionMethod = params.reductionMethod || 'tsne';
        ...
        post('log', { message: 'Running '+reductionMethod.toUpperCase()+' on '+m.length+' rows x '+m[0].length+' cols' });
        
        var embedding;
        if (reductionMethod === 'umap') {
          embedding = runUMAP(m, params.umap || {});
        } else {
          embedding = runTSNE(m, params.tsne || {});
        }
